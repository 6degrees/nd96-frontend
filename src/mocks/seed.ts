...
const SIGNATURES = [
    sig(
        'M18 72 C 48 38, 72 38, 95 58 C 118 78, 145 78, 168 52 C 188 32, 210 28, 235 48 C 248 58, 258 70, 278 36',
    ),
    sig('M22 60 C 55 20, 80 90, 120 48 S 175 18, 210 62 S 250 88, 282 42'),
    sig(
        'M16 68 C 40 30, 70 30, 95 62 C 115 88, 150 20, 185 55 C 210 80, 240 75, 275 40',
    ),
    sig(
        'M20 55 C 50 55, 60 85, 95 70 C 130 55, 140 25, 175 45 C 210 65, 230 30, 265 50 L 285 35',
    ),
    sig(
        'M24 75 C 45 25, 75 25, 100 70 C 120 100, 155 40, 190 60 C 220 78, 245 55, 280 45',
    ),
    sig(
        'M15 50 C 45 80, 75 80, 105 45 C 130 20, 160 20, 185 55 C 205 80, 240 70, 278 38',
    ),
    sig(
        'M28 70 C 55 45, 70 45, 90 65 C 110 85, 140 85, 165 55 C 185 35, 215 35, 245 60 L 275 42',
    ),
    sig(
        'M18 45 C 40 75, 70 90, 110 55 C 140 30, 170 30, 200 65 C 225 90, 255 60, 282 48',
    ),
    sig('M22 65 C 60 15, 95 95, 140 50 S 200 15, 250 70 L 285 40'),
    sig(
        'M16 58 C 35 40, 55 40, 75 60 C 95 80, 125 80, 150 50 C 170 28, 200 28, 230 55 C 250 72, 265 50, 285 44',
    ),
];
...
export function makeSeed(
    ...
    for (let i = 0; i < count; i++) {
        const language: Lang = i % 3 === 2 ? 'en' : 'ar';

        items.push({
            id: `msg_seed_${String(i).padStart(4, '0')}`,
            client_ref: `seed_ref_${String(i).padStart(4, '0')}`,
            ...
            department:
                i % 4 === 0
                    ? null
                    : {
                        value: pick(DEPARTMENTS, i).toLowerCase().replace(/\s+/g, '_'),
                        name_ar: pick(AR_NAMES, i),
                        name_en: pick(DEPARTMENTS, i),
                        label: pick(DEPARTMENTS, i),
                    },

            signature: {
                svg: pick(SIGNATURES, i),
                url: '',
            },
...
export function makeLiveMessage(
    n: number,
): ApiMessage {
    const language: Lang = n % 3 === 2 ? 'en' : 'ar';

    return {
        id: `msg_live_${String(n).padStart(4, '0')}`,
        client_ref: `live_ref_${String(n).padStart(4, '0')}`,
        ...
        department: {
            value: pick(DEPARTMENTS, n).toLowerCase().replace(/\s+/g, '_'),
            name_ar: '',
            name_en: pick(DEPARTMENTS, n),
            label: pick(DEPARTMENTS, n),
        },

        signature: {
            svg: pick(SIGNATURES, n + 5),
            url: '',
        },
