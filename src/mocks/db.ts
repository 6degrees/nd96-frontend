...
// Another tab wrote — swap in its snapshot.
...
// Demo word filter — the REAL list lives in Laravel.
export const BANNED_WORDS = ['badword', 'ممنوع'];

export function addMessage(
    msg: Omit<ApiMessage, 'id' | 'createdAt'>,
): ApiMessage {
    const full: ApiMessage = {
        ...msg,
        id: `msg_${String(db.nextId++).padStart(5, '0')}_${Math.random()
            .toString(36)
            .slice(2, 6)}`,
...
export function applyScreenCommand(
...
// localStorage.removeItem('nd96.mockdb.v6') + reload.
...
const REIGNS: ReignSeed[] = [
    ...
    [
        ...
        [
            [
                1385,
                'سافكو: أولى البتروكيماويات',
                'SAFCO: First Petrochemicals',
            ...
            [
                ...
                'The state raised its share in Aramco to 60% in 1974, paving the way to full ownership.',
            ],
        ],
    ],
    [
        ...
        [
            ...
            [
                1400,
                'الملكية الكاملة لأرامكو',
                'Full Ownership of Aramco',
    ...
    [
        ...
        [
            [
                1409,
                'إعلان أرامكو السعودية',
                'Saudi Aramco Established',
    ...
    [
        ...
        [
            ...
            [
                1448,
                'مشروع التوسعة «أميرال»',
                'The Amiral Expansion',
...
/**
 * تحويل بيانات seed الداخلية إلى الشكل المطلوب في TimelineDoc.
 * لا نغيّر الأنواع الأصلية، فقط أسماء المفاتيح في الداتا.
 */
function seedTimeline(): TimelineDoc {
    const reigns = REIGNS.map(
        ([hijriFrom, hijriTo, nameAr, nameEn, milestones], r) => ({
            id: `r${r + 1}`,

            name_ar: nameAr,
            name_en: nameEn,

            start_year: hijriFrom,
            end_year: hijriTo,
            ...
            // Reign type requires strings, not null.
            created_at: '2026-09-13T00:00:00.000Z',
            updated_at: '2026-09-13T00:00:00.000Z',

            milestones: milestones.map(
                ([year, titleAr, titleEn, bodyAr, bodyEn], m) => ({
                    id: `r${r + 1}m${m + 1}`,
...
export const timelineDb: { doc: TimelineDoc } = {
    doc: seedTimeline(),
};